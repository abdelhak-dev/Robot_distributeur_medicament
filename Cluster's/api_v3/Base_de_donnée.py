...
def position_robot(robotID,node):
    if type(robotID) != type(1):return {"code":200,"error": "robotID not correct"}
    if type(node) != type(1):return {"code":200,"error": "node not correct"}
    with connectBase() as conn:
            time = datetime.now()
            c = conn.cursor()
            # and now, insert 'new' record (really new or not)
            rSQL = ''' UPDATE MOUVEMENTS SET node = '{}', time = '{}'
                                  WHERE robotID = '{}';'''
            c.execute(rSQL.format(node,time,robotID))
            conn.commit()
            return {"code":200}
...
def getPosition(robotID:int):
    ...
    with connectBase() as conn:
        c = conn.cursor()
        # and now, insert 'new' record (really new or not)
        rSQL = '''SELECT node, time FROM MOUVEMENTS WHERE robotID = '{}';'''
        c.execute(rSQL.format(robotID))
        val = c.fetchall()
...
def setNode(node,booked):
    ...
    with connectBase() as conn:
        ...
        # in order to avoid replication, we control if this record already exists:
        # in this example, we try to delete directly the potential duplicate
        #SELECT node FROM MOUVEMENT
        rSQL = ''' INSERT INTO BOOKED (node, etat)
                                VALUES ('{}','{}');'''
        c.execute(rSQL.format(node,booked))

        rSQL = ''' UPDATE BOOKED SET node ='{}',
                                 etat ='{}';'''
        c.execute(rSQL.format(node, booked))

        rSQL2 = ''' UPDATE BOOKED SET etat ='{}' 
                                        WHERE node = '{}';'''
        c.execute(rSQL2.format(booked,node))
        conn.commit()
        return {"code":200}
...
"""def update_room_medicament(room:int):
    if type(room) != type(1):return "room not correct"
    with connectBase() as conn:
        c = conn.cursor()
