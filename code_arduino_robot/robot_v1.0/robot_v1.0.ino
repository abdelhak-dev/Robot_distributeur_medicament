...
// const char *ssid = "iPhone de Adrian";  // IDENTIFIANT
// const char *password = "06101997";  // MOT DE PASSE

// INFORMATIONS CONNEXION SERVEUR
String serverName = "http://10.3.7.41:8000/";
//String serverName = "http://172.20.10.14:8000/";
...
void initPosition(int a)
{
  ...
  {
    HTTPClient http;
    String serverPath = serverName + "setNode/" + a + "/" + "Yes";
...
int getOrder() {
  ...
  {
    HTTPClient http;
    String serverPath = serverName + "getOrder/" + num_robot;
...
String getPath( int room ) {
  if ( WiFi.status() == WL_CONNECTED )
  {
    ...
    if ( httpResponseCode > 0 )
    {
      ...
      Serial.println(payload);
      deserializeJson(output, payload);
      String getTrajetToDo = output["path"];
...
void setNode( char a, String etat )
{
  if ( WiFi.status() == WL_CONNECTED )
  {
    HTTPClient http;
    String serverPath = serverName + "setNode/" + a + "/" + etat;
    http.begin(serverPath.c_str());
    int httpResponseCode = http.GET();
    if ( httpResponseCode > 0 )
    {
...
String getNode( char pos )
{
  if ( WiFi.status() == WL_CONNECTED )
  {
    HTTPClient http;
    String serverPath = serverName + "getNode/" + pos ;
    http.begin(serverPath.c_str());
    int httpResponseCode = http.GET();
    if ( httpResponseCode > 0 )
    {
      ...
      Serial.println(payload);
      deserializeJson(output, payload);
      String isOccupied = output["booked"];
...
void arreter()
{
  ...
  digitalWrite(D4, HIGH); // MOTEUR GAUCHE : ARRIERE
  delay(20);
...
void loop() 
{
   ...
   * Tant que le numéro de chambre n'est pas valide, appeler la fonction getOrder()
   * Puis, afficher un message d'erreur
   * Enfin, attendre 2 secondes
   */
  ...
  if ( numeroChambre >= 1 && numeroChambre <= 4 )
  {
    ...
    /* REINITIALISER TABLE BOOCKED ( FONCTIONNALITE PROCHAINEMENT IMPLEMENTEE )
    for ( int i = 0 ; i < 9 ; i++ )
    {
      initPosition(i);
    }
    */
    ...
    while ( finTrajet == false )
    {
      ...
      switch ( etat )
      {
         ...
         * Tant que la prochaine intersection n'est pas libre, affecter à isFree la disponibilité de la prochaine intersection
         ...
         * Si l'intersection est 1, 2, 3 ou 4 alors, appeler la fonction donnerMedicament() puis passer la valeur de medicamentDonne à true
         ...
         */
        case 0 :
            while (isFree == "No") { isFree = getNode(trajetChambre[cpt_nombre+2]); }  // Yes = free & No = booked
         ...
         * Si ce caractère est un 'F' alors, appeler la fonction avancer() pendant 0,3 seconde dans le but faire traverser l'intersection
         * Sinon, appeler la fonction arreter() pendant 1 seconde dans le but de marquer l'intersection
         * Passer la valeur de etat à 0 avec comme paramètre la position actuelle dans la chaine de caractère
         * Puis, appeler la fonction setPosition() avec comme paramètre la position actuelle dans la chaine de caractère
         * Enfin, appeler deux fois la fonction setNode() avec comme premier paramètre la position actuelle dans la chaine de caractère, "No" et "Yes" en second paramètre dans le but de réserver et de libérer les intersections
         */
        case 1 :
            /* DETECTER OBSTACLE ( FONCTIONNALITE PROCHAINEMENT IMPLEMENTEE )
            ...
            */
            ...
            else if ( ( lnd == 1 ) && ( lng == 1 ) )
            {
              ...
              etat = 0;
              setPosition(trajetChambre[cpt_nombre]);
              setNode(trajetChambre[cpt_nombre], "No");
              setNode(trajetChambre[cpt_nombre - 2], "Yes");
            }
            break;
